...
- Pipeline 用 `define()` 显式构图，不再使用旧的 `forward()` 运行时拼装方案。
- Node 只保留真实 `op` 定义和输入输出契约，不再在代码里维护 `op_type`、业务 `category` 等冗余字段。
...
1. `envCheckNode` 校验 CANN 环境脚本是否存在。
2. `fetchModelNode` 从 Git 资源中获取 ONNX 模型。
3. `compileModelNode` 调用 `atc` 生成 `.om`。
4. `checkOmExistsNode` 校验输出文件是否生成。
...
5. `TestEngine` 逐节点执行 `EnvCheck / ResourceFetch / ATCCompile / PathExists`。
